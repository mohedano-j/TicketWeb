...
import { Employee, Project, Status, Ticket, TicketFromJSON } from "../../models";
...
export const TicketsPage = () => {
  ...
  const handleSave = (ticket: Ticket) => {
    ...
    if (ticket.ticketId === 0) {
      ticketProxy.add(ticket).subscribe((resp: Ticket) => {
        if (tickets) {
          setTickets([...tickets, { ...resp }]);
          setSelectedTicket(resp);
          toast.success("Ticket " + resp.title + "  added!");
        }
      });
    } else {
      ticketProxy.edit(ticket).subscribe((resp: Ticket) => {
        if (tickets) {
